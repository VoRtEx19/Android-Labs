...
public class MenuActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {                                                                   //overrode onclick
        Intent i;
        if (v.getId() == R.id.play) {//starts activity play and puts chosen gamerule
            i = new Intent(MenuActivity.this, ActivityPlay.class);
            if (gr[0] == null && gr[1] == null && gr[2] == null) gr[0] = "1";
            i.putExtra("gr", gr);
            i.putExtra("ls", scores[6]);
            startActivityForResult(i, 0);
        } else if (v.getId() == R.id.settings) {                                                                  //starts activity settings (puts already saved custom settings and gets new settings)
            i = new Intent(MenuActivity.this, ActivitySettings.class);
            i.putExtra("gameRule", gr);
            startActivityForResult(i, 1);
        } else if (v.getId() == R.id.leaderBoard) {                                                                  //gives a table for the leaderboard
            i = new Intent(this, ActivityLeaderBoard.class);
            i.putExtra("n", nicks);
            i.putExtra("s", scores);
            startActivity(i);
        } else if (v.getId() == R.id.exit) {//finishes the activity
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {       //gets gamerule settings from activity_settings
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == 0 && data != null) {
                ...
                int place = 6;
                for (int i = 5; i >= 0; i--)
                    if (Integer.parseInt(scoreResult != null ? scoreResult : "0") >= Integer.parseInt(scores[i])) place--;
                String tmpn, tmps;
